			  ...
			  if( (actualdistance < droneSearchRadius) ) {
				    //if probabilistic finding is turned off
			  		if(probabilisticRadius==false) {
					    ...
					    //Save ID to show which drone found the target
					  	collectedID.add(cdrone.droneID);
				  		foundTheTarget = true;
...
//				  			System.out.println("Successful find, but drone was faulted, total false negatives: " + numFalseNegatives);
	  ...
	  if (foundTheTarget) {
		  Startup_Multi.excelOutput.gatherData(drones, targets, collectedID, simCounter, Startup_Multi.numTrialsRun);
		  return 0;
	  }
	  else {
		  Startup_Multi.excelOutput.gatherData(drones, targets, simCounter, Startup_Multi.numTrialsRun);
		  return 1;
	  }
  }
    
    //Function for multiple targets
    //Currently Unused im like 90%sure
